...
export class SudokuEngine {
  ...
  solve(g:number[][]):boolean{ const pos=this.findEmpty(g); if(!pos) return true; const [r,c]=pos;
  ...
  countSolutions(g:number[][], limit=2){ let cnt=0; const dfs=()=>{ if(cnt>=limit) return; const p=this.findEmpty(g); if(!p){cnt++;return;} const [r,c]=p;
    for(let v=1; v<=this.N && cnt<limit; v++){ if(this.isValid(g,r,c,v)){ g[r][c]=v; dfs(); g[r][c]=0 } } }; dfs(); return cnt }
  ...
  isCompleteAndValid(g:number[][]){
    const N=this.N;
    for(let r=0;r<N;r++){ const R=new Set<number>(), C=new Set<number>(); for(let c=0;c<N;c++){ const vr=g[r][c], vc=g[c][r];
